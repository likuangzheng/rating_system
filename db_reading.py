...
def generate_ana_table(jj, seasonId):
    season_db = jj['seasons']#[ObjectId('58297abea723c1077adf5d8b')]
    seasonId = ObjectId(seasonId)
    season_info = list(season_db.find({"_id":seasonId}))#, 'status':'FINISHED'})
        ...
        for g in games:
            ...
            for p in peoples:
                if p['status'] == 'FINISHED':
                    ...
                    user_id = p['uid']
                    roles = {0:'C', 1:'E', 2:'D', 3:'F'}
                    user_name = list(jj['users'].find({'_id':ObjectId(p['uid'])}))[0]['name']
